...
RSpec.describe "チャットルームの削除機能", type: :system do
  ...
  it 'チャットルームを削除すると、関連するメッセージが全て削除されている' do
    ...
    #メッセージ情報を5つDBに追加する - DBカウントが5つ上がる      FactoryBot.create_list(:message, 5, room_id: @room_user.room.id, user_id: @room_user.user.id)

      FactoryBot.create_list(:message, 5, room_id: @room_user.room.id, user_id: @room_user.user.id)

    #「チャットを終了する」ボタンをクリックすることで、作成した5つのメッセージが削除されていることを確認する - DBカウントが5つ下がる
    expect {
